...
app.layout = html.Div(
    children=[
        ...
        dcc.Tabs(id='tabs', value='tab-1', children=[
            ...
            dcc.Tab(label='All Machines', value='tab-3'),
        ]),
...
def update_charts(tab, machine, machine_throughput, timeStart, timeEnd, lowerEnd, higherEnd):
    ...
    for index, row in filtered_data.iterrows():
        ...
        hover_text = (
            'Station: ' + machine +
            '<br>Time: ' + str(row[4]) +
            '<br>Duration: ' + str(row[2]) +
            '<br>Status Code: ' + str(row[3]) +
            ''.join(['<br>Post Status Code {}: {}'.format(j+1, code) for j, code in enumerate(post_status_codes)]) +
            ''.join(['<br>Previous Status Code {}: {}'.format(j+1, code) for j, code in enumerate(reversed(previous_status_codes))])
        )
