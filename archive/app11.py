...
def update_charts(machine, machine_throughput, timeStart, timeEnd, lowerEnd, higherEnd):
    #Machine selection
    ...
    for name, df, color in zip(machine_names, dataframes, colors):
        hover_text = 'Station: ' + name + '<br>Time: ' + df[4].astype(str) + '<br>Duration: %{y} <br>Status Code: ' + df[3].astype(str)
        ...
        "data": [
            {
                ...
                "y": filtered_data[2],
                "type": "lines",
                "hovertemplate": 'Time: %{x}<br>Duration: %{y} <br>Status Code: %{text}<extra></extra>',
                "text": filtered_data[3]
            },
        ],
        "layout": {
            "title": {
    ...
    return machine_performance_chart, machine_throughput, all_machines
