...
# Variable to detect whether or not the excel worksheet has been input
is_input_received = False
# Initialize data variable
data = None

# Function to load excel worksheet data
def load_excel_data():
    global is_input_received, data
    # If data has not been received
    if not is_input_received:
        # Prompt user to enter the file, account for the extension so they don't need to enter it
        file_name_without_extension = input("Please enter the excel worksheet name (without the '.xlsx' extension): ")
        # Append the file name with the proper excel worksheet extension
        file_name = f"{file_name_without_extension}.xlsx"
        # Print command to explain to the user the program is in the process of digesting the data since this takes some time
        print("Loading data...")
        # Try loading the data
        try:
            # Use the pandas command to ingest excel worksheet data
            # Drop row 1 because the machine data sometimes spikes on initialization; this can cause resulting graphs to expand wildly
            # Use ONLY columns B, C, and D, drop the rest
            ...
            # Data has been received, update flag
            is_input_received = True
            # Inform user data digest is finished
            print("Data loaded successfully")
            # Explain to the user that the application is being deployed, since kneading the data and serving the flask app takes time
            print("Loading application...")
        #Exception if file isn't found
        except FileNotFoundError:
            print(f"File '{file_name}' not found. Please check the file name and try again.")
        # Exception if file is read improperly or input data is improper to the tailored specs above
...
# Run the load data function (as seen above)
load_excel_data()

#Create all data frames contained in the excel sheet
# Developer Note: This can be reduced in size by utilizing a "for dataframe in dictionaryofdataframes" and possibly a zip function
...
# For each dataframe, drop the last row since it does not represent true data; like the first row the last can spike graphs wildly
# In addition, copy the 1st column to the 4th column (overwrite) for the purposes of timestamps on the X axis
# Developer Note: This can be reduced in size by using "for dataframe in listOfDataframes: drop the tail and overwrite the time
...
# Could be more efficient by once again using "dataframe in listOfDataframes: do thing"
...
# "for dataframe in listOfDataframes: subtract columns"
...
# List of strings for the selection of the primary station to examine
# These are directly the text the user sees when selecting the primary station
...
# List of strings for the selection of the throughout comparison station filter
# Once again plaintext for a user interface application
    ...
    children=[
        ...
        html.Div(
            children=[
                html.Div(
                    children=[
                    # Layout of the primary station filter
                ...
                html.Div(
                    children=[
                    # Layout of the secondary station filter
                ...
                html.Div(
                    children=[
                    # Layout of the Time Start filter
                        html.Div(
                ...
                # Layout of the Preset Filters button
...
# Function to convert a seconds value to a datetime value; useful for visually displaying a human readable X axis for all-machines graph
...
# Callback that handles the action of clicking the preset button
@app.callback(
...
# Function tied to the above callback that handles the action of clicking the preset button
def apply_preset_filters(n_clicks):
    if n_clicks is None:
        # No clicks yet, don't update the filters
        raise dash.exceptions.PreventUpdate

    # Preset filter values; these are arbitrarily chosen and can be changes to stakeholder desires
    ...
    # Update the filters to reflect the preset filters
...
# Callback to handle either tab selection or any filter changes to update the graph displayed/what data is displayed
@app.callback(
...
# Function tied to the above callback
def update_charts(tab, machine, machine_throughput, timeStart, timeEnd, lowerEnd, higherEnd):
    # Long else if statement handling which primary machine filter has been selected
    # Could possibly be refined by utilizing an above list of strings and index values with a loop
    ...
    # Copy the selected machine BEFORE data manipulation as to retain unmodified data to be displayed in previous/post status codes
    hover_text_copy = filtered_data.copy()
        
    # Like above, select the secondary machine to compare the primary machine to; could be refined
    ...
    # Create copies of all of the dataframes for use in the all-machines graph
    # Efficient iteration could be implemented
    ...
    # The following creates a numerical variable of the amount of times the "1 - Productive" status code is found within each hour
    # Each hour is represented in thousands of seconds, with the name of the number variable reflecting the hour range pulled from the filtered data
    # This is done by taking the len (size of) the filtered data between the given time period such that the code is productive
    # Essentially dropping all rows that are outside the timeframe and not containing the exact code shown
    ...
    # Lower bound filter that drops values that are below the duration value given
    # IE if you select "15 second" all data points with durations below 15 seconds will be dropped
    # Once again could be more efficient with iteration
    # This is utilized to drop "nominal" data or otherwise acceptable within the stakeholder's tolerance levels
    # In addition this can "squish" the Y-axis to make the graph more readable
    ...
    # Like the above block of codes, this section drops values with a duration greater than the given filter amount
    # This is utilized to drop outliers which may include invalid data and breaks
    if higherEnd == "1 hr":
    ...
    # Time start filter; drop values that are before the given timeframe
    # This is done in thousands of seconds due to datetime manipulation being finnicky in python
    # This also helps isolate data points to be more readable by "squishing" the X axis
    if timeStart == "6 AM":
    ...
    # Time end filter; like above except this drops data that exists after the given time filter
    if timeEnd == "6 AM":
        filtered_data = filtered_data[filtered_data[1] <= 21600]
        dfST10ACpy = dfST10ACpy[dfST10ACpy[1] <= 21600]
        dfST20ACpy = dfST20ACpy[dfST20ACpy[1] <= 21600]
    ...
    machinesLines = []
    # Station labels for use in the all-machines graph including the legend
    ...
    # Define a list of colors for the lines to differentiate different stations/machines lines
    ...
    # List of corresponding dataframes used in the all-machines graph
    ...
    # Append lines for each machine by iterating over the dataframes, utilizing the names and colors to zip the lines into one graph
    for name, df, color in zip(machine_names, dataframes, colors):
        
        # Convert the seconds to date time (note this is using an above function)
        time_values = df[1].apply(seconds_to_datetime)
        # Create the hover text with the given dataframe (station)
        hover_text = (
        'Station: ' + name + 
        ...
        # Append this new station line to the graph
